...
export function useDrafts() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("jazz-forge-drafts");
  ...
  function saveDraft(draft: Omit<Draft, "id" | "createdAt" | "starred">) {
    ...
    setDrafts(updated);
    localStorage.setItem("jazz-forge-drafts", JSON.stringify(updated));
    return newDraft;
  }
  ...
  function clearDrafts() {
    setDrafts([]);
    localStorage.removeItem("jazz-forge-drafts");
  }
