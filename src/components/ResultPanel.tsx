  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: "8px", overflow: "hidden", marginBottom: "10px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border)" }}>
        <span style={{ fontSize: "10px", fontWeight: 500, color: "var(--gold)", letterSpacing: "0.1em", textTransform: "uppercase" }}>{label}</span>
        <div style={{ display: "flex", gap: "6px" }}>
          <button onClick={() => setShowRefine(!showRefine)} style={{
            fontSize: "11px",
            color: showRefine ? "var(--gold)" : "var(--text-muted)",
            background: showRefine ? "#1A1508" : "transparent",
            border: `1px solid ${showRefine ? "var(--gold-dim)" : "var(--border)"}`,
            borderRadius: "4px", padding: "3px 8px", cursor: "pointer", transition: "all 0.15s",
          }}>Refine</button>
          <button onClick={handleCopy} style={{
            fontSize: "11px", color: copied ? "var(--gold)" : "#F0EDE6",
      ...
      {showRefine && (
        <div style={{ padding: "10px 14px", background: "#110E05", borderBottom: "1px solid var(--border)", display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <button onClick={handleRefine} disabled={refining || !instruction.trim()} style={{
            padding: "6px 14px",
            background: refining || !instruction.trim() ? "var(--bg-card)" : "var(--gold)",
      ...
      <div style={{ padding: "14px", background: "var(--bg-card)" }}>
        {localContent.split("\n").map((line, i) => {
          ...
          if (/^\[.+\]$/.test(trimmed)) return (
            <div key={i} style={{ fontSize: "10px", fontWeight: 500, color: "var(--gold)", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: "12px", marginBottom: "4px" }}>
              {trimmed.replace(/[\[\]]/g, "")}
            </div>
          );
...
export default function ResultPanel({ result, loading, isStreaming }: ResultPanelProps) {
  const title = result.split("\n").find(l => /^#?\s*TITLE:/i.test(l))?.replace(/^#?\s*TITLE:/i, "").trim() || "jazz-forge";
