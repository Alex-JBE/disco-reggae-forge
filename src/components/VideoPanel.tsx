...
export default function VideoPanel({ title, style, mood, theme, composition, compositionLoading, onResult }: VideoPanelProps) {
  ...
  const segBtnStyle = (active: boolean) => ({
    ...
    padding: "5px 4px",
    fontSize: "10px",
    background: active ? "var(--border-gold)" : "var(--bg-card)",
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "16px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ fontSize: "10px", letterSpacing: "0.12em", color: "var(--gold-dim)", textTransform: "uppercase", marginBottom: "4px" }}>Video Script</div>
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: "10px" }}>
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              {CLIP_LENGTHS.map((l) => (
                <button key={l} onClick={() => setClipLength(l)} style={{
                  flex: 1, padding: "6px",
                  background: clipLength === l ? "var(--border-gold)" : "var(--bg-card)",
                  border: `1px solid ${clipLength === l ? "var(--gold-dim)" : "var(--border)"}`,
                  borderRadius: "6px",
                  color: clipLength === l ? "var(--gold-light)" : "var(--text-muted)",
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              {["6", "8", "10", "12", "16"].map((n) => (
                <button key={n} onClick={() => setSceneCount(n)} style={{
                  flex: 1, padding: "6px",
                  background: sceneCount === n ? "var(--border-gold)" : "var(--bg-card)",
                  border: `1px solid ${sceneCount === n ? "var(--gold-dim)" : "var(--border)"}`,
                  borderRadius: "6px",
                  color: sceneCount === n ? "var(--gold-light)" : "var(--text-muted)",
                  fontSize: "12px", cursor: "pointer", fontFamily: "'DM Mono', monospace",
                }}>
                  {n}
                </button>
        ...
        {previewClips !== null && previewClips > 0 && !result && (
          <div style={{ padding: "7px 10px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "6px", fontSize: "11px", color: "var(--gold-dim)" }}>
            {duration} ÷ {clipLength}s = <span style={{ color: "var(--gold)", fontWeight: 500 }}>{previewClips} clips</span>
          </div>
        )}
      </div>
            ...
            <div style={{ padding: "0 16px 12px", display: "flex", flexDirection: "column", gap: "8px" }}>
              {[
                ...
                <div key={label} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "6px", overflow: "hidden" }}>
                  <div style={{ padding: "5px 10px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border)", fontSize: "9px", color: "var(--gold)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
                    {label}
                  </div>
      ...
      {clips.length > 0 && (
        <div style={{ padding: "8px 16px", borderBottom: "1px solid var(--border)", display: "flex", gap: "16px", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: "13px", color: "var(--gold)", fontFamily: "'DM Mono', monospace" }}>{duration || "—"}</div>
          </div>
          <div>
            <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "0.06em" }}>Clips</div>
            <div style={{ fontSize: "13px", color: "var(--gold)", fontFamily: "'DM Mono', monospace" }}>{clips.length}</div>
          </div>
          <div>
            <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "0.06em" }}>Mode</div>
            <div style={{ fontSize: "11px", color: "var(--gold)", fontFamily: "'DM Mono', monospace" }}>
              {segMode === "split_by_duration" ? `${clipLength}s` : segMode === "split_by_scene_count" ? `${sceneCount} sc` : "full"}
            </div>
          </div>
          <div style={{ marginLeft: "auto" }}>
            <button onClick={copyAll} style={{ padding: "4px 10px", background: "transparent", border: "1px solid var(--border)", borderRadius: "4px", color: copied === "all" ? "var(--gold)" : "#F0EDE6", fontSize: "11px", cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "12px 16px" }}>
        {loading && (
          <div style={{ color: "var(--text-muted)", fontSize: "12px", textAlign: "center", paddingTop: "24px" }}>
            <span style={{ color: "var(--gold)" }}>●</span> Writing scene prompts...
          </div>
        )}
        {!loading && !result && (
            ...
            {clips.map((clip) => (
              <div key={clip.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "6px", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border)" }}>
                  <span style={{ fontSize: "10px", color: "var(--gold)", fontFamily: "'DM Mono', monospace", letterSpacing: "0.06em" }}>
                    CLIP {clip.id.padStart(2, "0")}
                  </span>
                  <button onClick={() => copy(clip.prompt, clip.id)} style={{ fontSize: "10px", color: copied === clip.id ? "var(--gold)" : "#F0EDE6", background: "transparent", border: "1px solid #888578", borderRadius: "4px", padding: "2px 6px", cursor: "pointer" }}>
                    {copied === clip.id ? "✓" : "Copy"}
                  </button>
                </div>
                <div style={{ padding: "8px 10px", fontSize: "11px", color: "var(--text-secondary)", lineHeight: "1.6" }}>
      ...
      <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", display: "flex", gap: "6px" }}>
        ...
        <button
          ...
          style={{
            flex: 1, padding: "10px",
            background: isDisabled ? "var(--bg-card)" : "var(--border-gold)",
            border: "1px solid var(--gold-dim)", borderRadius: "6px",
            color: isDisabled ? "var(--text-muted)" : "var(--gold-light)",
