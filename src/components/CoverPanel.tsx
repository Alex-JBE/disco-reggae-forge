  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "16px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ fontSize: "10px", letterSpacing: "0.12em", color: "var(--gold-dim)", textTransform: "uppercase", marginBottom: "4px" }}>Cover Art</div>
          ...
          {FORMATS.map((f) => (
            <div key={f.key} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <span style={{ color: "var(--gold)", fontSize: "12px" }}>{f.icon}</span>
          ...
          {IMAGE_SERVICES.map(s => (
            <button
              ...
              style={{
                ...
                borderRadius: "6px",
                border: `1px solid ${activeService === s.id ? "#C9A84C" : "#2A2A30"}`,
                background: activeService === s.id ? "#1A1508" : "#1A1A1F",
                color: activeService === s.id ? "#C9A84C" : "#888578",
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "12px 16px" }}>
        {loading && (
          <div style={{ color: "var(--text-muted)", fontSize: "12px", textAlign: "center", paddingTop: "24px" }}>
            <span style={{ color: "var(--gold)" }}>●</span> Generating prompts...
          </div>
        )}
        {!loading && !result && (
            ...
            {FORMATS.map((f) => parsed[f.key] ? (
              <div key={f.key} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "8px", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderBottom: "1px solid var(--border)", background: "var(--bg-secondary)" }}>
                  <span style={{ fontSize: "10px", color: "var(--gold)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{f.label} · {f.ratio}</span>
                  <div style={{ display: "flex", gap: "5px" }}>
                    <button
                      onClick={() => copy(parsed[f.key], f.key)}
                      style={{ fontSize: "10px", color: copied === f.key ? "var(--gold)" : "#F0EDE6", background: "transparent", border: "1px solid #888578", borderRadius: "4px", padding: "2px 8px", cursor: "pointer" }}
                    >
                    ...
                    <button
                      ...
                      style={{
                        fontSize: "10px",
                        color: copied === `open_${f.key}` ? "#5DCAA5" : "#C9A84C",
                        background: copied === `open_${f.key}` ? "#0F3028" : "#1A1508",
                        border: `1px solid ${copied === `open_${f.key}` ? "#1D9E75" : "#C9A84C"}`,
